...
int load_dataset(const char *img_path, const char *lbl_path, Dataset *out) {
    ...
    if (!fi||!fl) { perror("fopen"); return -1; }
    uint32_t magic,n,rows,cols;
    SAFE_FREAD(&magic,4,1,fi); SAFE_FREAD(&n,4,1,fi);
    SAFE_FREAD(&rows,4,1,fi); SAFE_FREAD(&cols,4,1,fi);
    magic=be32(magic); n=be32(n);
    rows=be32(rows); cols=be32(cols);
    if(magic!=0x00000803) return -2;
    SAFE_FREAD(&magic,4,1,fl); SAFE_FREAD(&n,4,1,fl);
    magic=be32(magic); n=be32(n);
    if(magic!=0x00000801) return -3;
...
void free_dataset(Dataset *d) {
    ...
    for(uint32_t i=0;i<d->count;++i) free(d->images[i]);
    free(d->images);
    free(d->labels);
}